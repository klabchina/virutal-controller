...
{
    /// <summary>
    /// CarouselのVertical用View(NoLoop版)
    /// </summary>
    public class CarouselViewVertical : CarouselViewNoLoopBase
    {
#region override methods
        ...
        {
            return new Vector3(ContentLocalPosition.x, tempDelta + contentBasePos.y, ContentLocalPosition.z);
        }
        ...
        {
            return new Vector3(0f, (this.property.CellSizeYValue + this.property.CellSpacingYValue) * -offset);
        }
        ...
        public override Vector3 DistanceToCenteringPosition()
        {
            ...
            var distance = startTempDelta - offsetTo.y - tempDelta;
        ...
        {
            if (tempDelta + delta.y < startTempDelta)
            {
                // Prevの場合はstartTempDeltaのところが限界位置となるので、startTempDeltaからtempDeltaを引いた値を返す
                return new Vector3(delta.x, startTempDelta - tempDelta, delta.z);
            }
            return delta;
        }
        ...
        {
            var maxDelta = startTempDelta - GetAmountOfMovement(this.property.CellCount - 1).y;
            if (tempDelta + delta.y > maxDelta)
            {
                // Nextの場合はstartTempDeltaからContentのサイズ*CellCount分が限界位置となるので、maxDeltaからtempDeltaを引いた値を返す
                return new Vector3(delta.x, maxDelta - tempDelta, delta.z);
            }
            return delta;
        }
...
#region protected methods
        ...
        protected override Vector3 CalculateContentBasePosition(int cellCount)
        {
            ...
            var evenPosition = new Vector3(0f, halfPosition.y, 0f);
