...
{
    [Serializable]
    public class ListModelVertical : ListModelBase
    {
...
#region Fields & Properties
        ...
        public override float ContentPreferredSize
        {
            get
            {
                ...
                return Padding.Top + Padding.Bottom + (CellSize + Spacing) * count - Spacing;
            }
        }
...
#region Information API
        ...
        {
            var count = VisibleUnitCount(ViewportSize.y, Padding.Top, Padding.Bottom, CellSize, Spacing, extendable);
            if (extendable)
            {
                return count + 1;
            }
        ...
        {
            if (span < Padding.Top + CellSize + Spacing)
            {
                return 0;
            }
            return (int) Math.Floor((span - Padding.Top) / (CellSize + Spacing));
        }
        ...
        public override float CellPosition(int index)
        {
            if (index < 0)
            {
                var message = string.Format("[ListModelVertical] Illegal index:{0}", index);
                throw new ArgumentOutOfRangeException(message);
            }
            return Padding.Top + (CellSize + Spacing) * index + CellSize * (1 - CellPivot.y);
        }
